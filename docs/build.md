...
- **LinguistTools introuvable** : `lupdate` / `lrelease` viennent de l’archive Qt de base `qttools` (déjà dans un kit desktop aqt). Ne pas passer `-m qttools` : ce n’est plus un module extra (aqt échoue).
