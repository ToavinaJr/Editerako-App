...
| `editor/` | `EditerakoEditor` | `CodeEditor`, `EditorDocument`, `EditorArea` / `EditorGroup` (split, pin, preview), `TabOps`, `EditorManager`, `EditorIo` / `EditorStyle` / `HighlighterSync` / `EditorStatusWidget` / `StatusBarText` ; `features/` (dont folding) |
