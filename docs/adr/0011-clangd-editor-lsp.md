...
- clangd : spec `clangd` + `--offset-encoding=utf-16`. Si `compile_commands.json` est trouvé (`build/<preset>/`, etc.), il est passé via `--compile-commands-dir` — sinon clangd ne voit pas les `-I` CMake et rétablit les types inconnus en `int`. Hover rendu en HTML (templates échappés, couleurs C++, lien Go to Definition). `Ctrl+clic` → définition ; `Alt+clic` → curseur extra. `initialize` seulement après `QProcess::started`. Binaire absent : message status bar, pas de crash.
